...
> **Phase F ✅ released (v1.5.2):** `reusable-config-ci.yml` (yamllint + `docker compose config` + gitleaks + OPA/conftest, dual-gate, runner-label-driven) für die compose-only Infra-Repos `paperless` + `cloudflare`; je ~20-Zeilen `ci.yml`-Thin-Caller. Smoke `_smoke-config-ci.yml` grün auf ubuntu gegen `tests/fixtures/config`. Plan: `docs/superpowers/plans/2026-06-03-ci-powerup-phaseF-config-ci.md`.
> **Offen C, E, D (je eigener writing-plans→exec-Zyklus):** C = frontend a11y(pa11y/axe) in `reusable-frontend.yml` (Lighthouse existiert schon aus Cycle B) · E = GitHub-Environments+Prod-Approval+PR-concurrency (⚠️ Environment-protection evtl. paid-gated auf private) · D = API-Contract/schemathesis (⚠️ braucht OpenAPI-Specs pro App = größter Brocken, App-Code-Folgearbeit außerhalb der CI-Spur).
>
